...
class _WeatherPageState extends State<WeatherPage> {
  ...
  Future<WeatherResponse?> getWeather(Position currentPosition) async {
    ...
    try {
      ...
      if (listAddress.isNotEmpty) {
        currentAddress =
            '${listAddress.first.subAdministrativeArea}, ${listAddress.first.administrativeArea}';
      }
    ...
    if (response.result == true) {
      ...
      if (kDebugMode) {
        print(
            "Weather code: ${response.code} - Weather message: ${response.message}");
      }
  ...
  Future<Position> getLocation() async {
    ...
    if (locationPermission == LocationPermission.deniedForever) {
      idErrorLocation = 2;
      return Future.error(
          "Location permission are permanently denied, can't request location");
    }
          ...
          return FutureBuilder(
            ...
            builder: (context, snapshotWeather) {
              if (snapshotWeather.hasData) {
                List weatherCodeDay =
                    snapshotWeather.data!.hourly!.weathercode!.sublist(0, 24);
                List temperatureDay =
                    snapshotWeather.data!.hourly!.temperature2m!.sublist(0, 24);
                List timeDay =
                    snapshotWeather.data!.hourly!.time!.sublist(0, 24);

                List<Map<String, dynamic>> dataDay = List.generate(
                  weatherCodeDay.length,
                  (index) {
                    Map weather = AppConstant.listWMOCode.firstWhere(
                        (element) =>
                            element['code'] == (weatherCodeDay[index] ?? -1));
                ...
                String weatherStatus = AppConstant.listWMOCode.firstWhere(
                    (element) =>
                        element['code'] ==
                        (snapshotWeather.data!.currentWeather!.weathercode ??
                            -1))['desEN'];
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color:
                                      const Color(0xFFC3C3C3).withOpacity(0.2),
                              ...
                              for (int i = 0;
                                  i < snapshotWeather.data!.daily!.time!.length;
                                  i++)
                                Container(
                                  margin: const EdgeInsets.only(
                                    left: 30,
                ...
                debugPrint("${snapshotWeather.error}");
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            "Error: ${snapshotWeather.error.toString()}",
                            textAlign: TextAlign.center,
                            style: const TextStyle(
                              color: Colors.red,
                              fontWeight: FontWeight.w500,
